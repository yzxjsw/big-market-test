...
/**
 * AbstractRaffleActivityPartakeService
 *抽奖活动参与抽奖类
 * @author yezhenxiong
 * @version 2024/08/09 21:44
 **/
...
public abstract class AbstractRaffleActivityPartake implements IRaffleActivityPartakeService {
    ...
    public UserRaffleOrderEntity createOrder(PartakeRaffleActivityEntity partakeRaffleActivityEntity) {
        ...
        if (null != userRaffleOrderEntity) {
            log.info("创建参与活动订单[未消费] userId:{} activityId:{} userRaffleOrderEntity:{}", userId, activityId, JSON.toJSONString(userRaffleOrderEntity));
            return userRaffleOrderEntity;
        }
        // 3. 额度账户过滤【判断用户的额度是否充足】&返回账户构建对象
        CreatePartakeOrderAggregate createPartakeOrderAggregate = this.doFilterAccount(userId, activityId, currentDate);

        // 4. 构建订单
        UserRaffleOrderEntity userRaffleOrder = this.buildUserRaffleOrder(userId, currentDate, activityEntity);
    ...
    protected abstract UserRaffleOrderEntity buildUserRaffleOrder(String userId, Date currentDate, ActivityEntity activityEntity);
}
